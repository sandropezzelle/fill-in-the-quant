# Based on https://github.com/flomlo/ntm_keras , downloaded on 2018-02-02
# Minor changes made by Shane Steinert-Threlkeld
...
def build_ntm_model(params):

    # initialization
    model = Sequential()
    # TODO: build controller based on keyword in params
    model.name = 'NTM_{}-controller'.format(params['controller'])
    model.batch_size = params['batch_size']
    model.input_dim = params['embedding_dim']
    model.output_dim = params['num_classes']

    # controller
    controller_input, controller_output = controller_input_output_shape(
    ...
    ntm = NeuralTuringMachine(params['num_classes'],
                              ...
                              write_heads=params['write_heads'],
                              return_sequences=False,  # for classification
                              input_shape=(None, params['embedding_dim']),
